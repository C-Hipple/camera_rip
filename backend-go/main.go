...
var supportedBrands = []cameraBrand{
	...
	{suffix: "CANON", rawExt: ".CR2"},
	{suffix: "OLYMP", rawExt: ".ORF"},
	{suffix: "SONY", rawExt: ".ARW"},
	{suffix: "NIKON", rawExt: ".NEF"},
	{suffix: "FUJI", rawExt: ".RAF"},
	{suffix: "PANA", rawExt: ".RW2"},
}
...
func getPhotosHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, file := range files {
		if !file.IsDir() {
			...
			if (isImage || isRaw) && !strings.HasPrefix(file.Name(), "._") {
...
func extractRawPreview(directory, filename string) (string, error) {
	// Check cache first
	previewDir := filepath.Join(rawPreviewCacheDir, directory)
	...
	rawFilePath := filepath.Join(photoBaseDir, directory, filename)
...
func generateThumbnail(directory, filename string) error {
	...
	if isRawFile(filename) {
		...
		sourceImagePath = previewPath
	} else {
		sourceImagePath = filepath.Join(photoBaseDir, directory, filename)
	}
